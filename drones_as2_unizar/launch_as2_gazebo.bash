...
if [[ ${swarm} == "true" ]]; then
  simulation_config="config/gazebo/world_swarm.yaml"
else
  simulation_config="config/gazebo/world.yaml"
fi
...
for namespace in ${drone_namespaces[@]}; do
  # Only launch simulation for the first namespace
  base_launch="false"
  if [[ ${namespace} == ${drone_namespaces[0]} && ${launch_simulation} == "true" ]]; then
    base_launch="true"
  fi
  eval "tmuxinator ${tmuxinator_mode} -n ${namespace} -p tmuxinator/gazebo/aerostack2.yaml \
